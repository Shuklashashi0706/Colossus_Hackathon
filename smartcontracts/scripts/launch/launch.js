...
async function phygitalADeploy() {
    const constructorParam = jsonContent.constructorParams
    const PhygitalA = await ethers.getContractFactory("PhygitalA")
    console.log("PhygitalA deploy function called")
    ...
    // Check and print deployer balance
    const [deployer] = await ethers.getSigners()
    const deployerBalance = await deployer.getBalance()
    console.log(
        `Deployer balance before deployment: ${ethers.utils.formatEther(
            deployerBalance
        )} ETH`
    )

    // Estimate and print deployment cost
    const estimatedCost = options.gasPrice.mul(options.gasLimit)
    console.log(
    ...
    if (deployerBalance.lt(estimatedCost)) {
        console.error(
            "Insufficient funds for deployment. Please add more ETH to your account."
        )
        return
    }
